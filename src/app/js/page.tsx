...
export default function JSMentorPage() {
  const scene = SCENES.javascript[0];
  const { sharedMode, sharedConversation } = useShareParams();
  const { conversation, loading, restoring, error, sendMessage, deleteExchange, clearConversation } = useChat("JavaScript");
  ...
  function handleGiveUp() {
    return sendMessage("我卡住了，请直接告诉我答案。", "", "JavaScript", { giveUp: true });
  }
  ...
  return (
    <div style={{ height: "100vh", display: "flex", flexDirection: "column" }}>
      ...
      <header className="cm-lab-header">
        ...
        <span className="cm-lab-title">🟨 JavaScript 实验排错导师舱</span>
      ...
      <div className="cm-lab-layout">
        <div className="cm-lab-left">
          <CodeInputPanel onSend={sharedMode ? undefined : sendMessage} loading={loading} language="JavaScript" />
        </div>
