...
export default function JavaMentorPage() {
  const scene = SCENES.java[0];
  const { sharedMode, sharedConversation } = useShareParams();
  const { conversation, loading, restoring, error, sendMessage, deleteExchange, clearConversation } = useChat("Java");
  const [showConfirmClear, setShowConfirmClear] = useState(false);

  // Use the actual conversation being displayed to ChatInterface
  ...
  function handleGiveUp() {
    return sendMessage("我卡住了，请直接告诉我答案。", "", "Java", { giveUp: true });
  }
  ...
  return (
    <div style={{ height: "100vh", display: "flex", flexDirection: "column" }}>
      ...
      <header className="cm-lab-header">
        ...
        <span className="cm-lab-title">☕ Java 实验排错导师舱</span>
      ...
      <div className="cm-lab-layout">
        <div className="cm-lab-left">
          <CodeInputPanel onSend={sharedMode ? undefined : sendMessage} loading={loading} language="Java" />
        </div>
